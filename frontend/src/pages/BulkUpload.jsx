...
const BulkUpload = () => {
  ...
  useEffect(() => {
    ...
    if (!storedUser) {
      ...
      if (userData.userType !== 'faculty' && userData.userType !== 'admin') {
        navigate('/dashboard');
      }
      setUser(userData);
    }
  ...
  const handleFilesChange = (e) => {
    ...
    if (validFiles.length !== selectedFiles.length) {
      setError(`${selectedFiles.length - validFiles.length} file(s) skipped (invalid type or size > 10MB)`);
    } else {
      setError('');
    }
  ...
  const handleBulkUpload = async () => {
    ...
    try {
      ...
      const response = await uploadAPI.uploadBulkFiles(formData);
  ...
  const handleSaveSelectedQuestions = async () => {
    ...
    try {
      ...
      setUploadResults({
        message: `Successfully saved ${selectedQuestions.length} question(s)!`,
        results: [{
          status: 'success',
          fileName: selectedDocForGeneration.fileName,
          textLength: selectedQuestions.length
        }]
      });
      ...
      <header className="bg-gradient-to-r from-primary-500 to-secondary-500 text-white shadow-lg">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-5">
          <div className="flex justify-between items-center">
            <div>
              <h1 className="text-xl sm:text-2xl font-bold">Bulk File Upload</h1>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {quota && (
          <div className="bg-white rounded-xl shadow-md p-6 mb-6">
            <h3 className="text-lg font-semibold text-gray-800 mb-4">Faculty Quota</h3>
        ...
        {/* Bulk Upload Form */}
        <div className="bg-white rounded-xl shadow-md p-6 mb-6">
          <h3 className="text-lg font-semibold text-gray-800 mb-4">Upload Multiple Files</h3>

          <div className="space-y-4">
            {/* File Input */}
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">
                Select Files (PDF, DOCX, or TXT) - Up to 50 files
              </label>
              <input
                id="bulkFileInput"
                type="file"
              ...
              {files.length > 0 && (
                <div className="mt-3 p-3 bg-blue-50 border border-blue-200 rounded-lg">
                  <div className="text-sm font-semibold text-blue-800 mb-2">
                    {files.length} file(s) selected ({formatBytes(getTotalFilesSize())})
                  </div>
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">
                Subject (Optional - applies to all files)
              </label>
              <input
                type="text"
                value={subject}
                ...
                {uploadResults.results.map((result, idx) => (
                  <div key={idx} className={`text-sm p-2 rounded ${
                    ...
                    {result.status === 'success' ? (
                      <span className="ml-2">✓ Uploaded ({result.textLength} chars extracted)</span>
                    ) : (
                      <span className="ml-2">✗ Failed: {result.error}</span>
                    )}
                  </div>
                ))}
              </div>
